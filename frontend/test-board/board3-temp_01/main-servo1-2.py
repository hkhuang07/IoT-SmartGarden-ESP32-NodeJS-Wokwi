...
FIXED:
1. Button Logic: Buttons now only toggle the 'auto_mode' status (Manual Override).
2. Water Valve Logic: Guaranteed ON/OFF (100%/0%) state in Auto Mode.
3. Servo 2 (Roof) Behavior: Added a small delay in set_roof_percentage to simulate slower movement.

Author: MiniMax Agent
Version: INTEGRATED & LOGIC ADDED - 2025-12-02
"""
...
# Global hardware objects for buttons/LEDs
...
def setup_servos():
    """Setup servo motors - FIX: Sử dụng dải 40-115"""
...
def set_water_percentage(percentage):
    """Set water valve percentage (0-100) using 10-bit duty cycle (40-115)"""
    global water_percentage, servo_water
    
    if not servo_water: return
        
    percentage = max(0, min(100, percentage))
    # BỎ tối ưu hóa để đảm bảo lệnh luôn được gửi khi logic auto yêu cầu
    ...
    print(f"💧 Water: {percentage}% (duty:{int(duty)} - 10bit)")
    update_lcd_display()

def set_roof_percentage(percentage):
    """Set roof percentage (0-100) using 10-bit duty cycle (40-115) - FIX: Added small delay"""
    ...
    # --- SIMULATE SLOWER MOVEMENT (FIX) ---
    if abs(percentage - roof_percentage) > 10:
        # Nếu thay đổi lớn, mô phỏng quá trình đóng/mở chậm (ví dụ: 100ms delay)
        time.sleep_ms(100) 
    ...
    print(f"🏠 Roof: {percentage}% (duty:{int(duty)} - 10bit)")
    update_lcd_display()
        ...
        if topic_str == MQTT_SENSOR_DATA_TOPIC:
            ...
            try:
                ...
                if temp is not None and hum is not None:
                    ...
                    # Kích hoạt logic tự động ngay sau khi nhận dữ liệu mới
                    if auto_mode:
                        auto_control_logic() 
                    update_lcd_display()
                else:
...
def auto_control_logic():
    ...
    Van nước: Đảm bảo LUÔN BẬT (100%) hoặc LUÔN TẮT (0%) dựa trên độ ẩm.
    """
    ...
    # --- LOGIC ĐIỀU KHIỂN NHIỆT ĐỘ (MÁI CHE) ---
    if current_temperature > THRESHOLD["temperature"]["max"]:
        print(f"🔥 TEMP HIGH ({current_temperature:.1f} > {THRESHOLD['temperature']['max']}). Opening roof.")
        if roof_percentage != 100:
            set_roof_percentage(100)
    
    elif current_temperature < THRESHOLD["temperature"]["min"]:
        print(f"❄️ TEMP LOW ({current_temperature:.1f} < {THRESHOLD['temperature']['min']}). Closing roof.")
        if roof_percentage != 0:
            set_roof_percentage(0)
    
    # --- LOGIC ĐIỀU KHIỂN ĐỘ ẨM (VAN NƯỚC/HUMIDIFIER) ---
    
    if current_humidity < THRESHOLD["humidity"]["min"]:
        # ĐIỀU KIỆN 1: Độ ẩm quá thấp -> Yêu cầu MỞ VAN (BẬT 100%)
        if water_percentage != 100:
            set_water_percentage(100)
            print("💧 ACTION: Van nước BẬT (100%)")
            
    else:
        # ĐIỀU KIỆN 2: Độ ẩm ổn định hoặc quá cao (>= 50.0%) -> Yêu cầu ĐÓNG VAN (TẮT 0%)
        if water_percentage != 0:
            set_water_percentage(0)
            print("💦 ACTION: Van nước TẮT (0%)")
...
def check_buttons():
    """Check button states (Manual Override) - FIX: Chỉ chuyển đổi chế độ tự động"""
    global water_percentage, roof_percentage, auto_mode
    
    try:
        # Water button (Pin 25, active low) - Toggle Auto Mode
        if water_button and not water_button.value():
            # Chuyển đổi chế độ tự động global
            auto_mode = not auto_mode
            
            # Nếu chuyển sang thủ công, thiết lập Van nước về 0% (trạng thái an toàn)
            if not auto_mode:
                 set_water_percentage(0)
                 print("🔘 Button 1: Van nước MANUAL MODE. Van TẮT.")
            else:
                 print("🔘 Button 1: Van nước AUTO MODE.")

            time.sleep_ms(300) 
        
        # Roof button (Pin 26, active low) - Toggle Auto Mode
        if roof_button and not roof_button.value():
            # Chuyển đổi chế độ tự động global
            auto_mode = not auto_mode
            
            # Nếu chuyển sang thủ công, thiết lập Mái che về 0% (trạng thái an toàn)
            if not auto_mode:
                set_roof_percentage(0)
                print("🔘 Button 2: Mái che MANUAL MODE. Mái che ĐÓNG.")
            else:
                print("🔘 Button 2: Mái che AUTO MODE.")

            time.sleep_ms(300) 

        # Mode button (Pin 27, active low) - Toggle Auto Mode (Global)
        if mode_button and not mode_button.value():
            auto_mode = not auto_mode
            
            if not auto_mode:
                # Nếu tắt Auto Mode, đặt cả hai Servo về trạng thái an toàn (0%)
                set_water_percentage(0)
                set_roof_percentage(0)
                print("🔄 Mode Button: GLOBAL MANUAL MODE")
            else:
                print("🔄 Mode Button: GLOBAL AUTO MODE")
